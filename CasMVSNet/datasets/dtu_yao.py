...
class MVSDataset(Dataset):
    ...
    def read_mask_hr(self, filename):
        ...
        h, w = np_img.shape
        np_img_ms = {
            "stage1": cv2.resize(np_img, (w//4, h//4), interpolation=cv2.INTER_NEAREST),    # 168*120
            "stage2": cv2.resize(np_img, (w//2, h//2), interpolation=cv2.INTER_NEAREST),    # 320*256
            "stage3": np_img,                                                               # 640*512
        }
        return np_img_ms
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            proj_mat = np.zeros(shape=(2, 4, 4), dtype=np.float32)  # 只是把内外参存到一个tensor里，并没有乘起来
